...
class CO():
    ...
    def train(self, df, appliance_list):
        ...
        * only uses first chunk for each meter (TODO: handle all chunks).
        ...
        for i, meter in enumerate(appliance_list):
            if self.debug:
                print(" [train] Training model for submeter", meter)
            meter_data = df[meter]

            num_on_states = None
            self.train_on_chunk(meter_data, meter,
                                max_num_clusters, num_on_states)
    ...
    def disaggregate(self, df):
        ...
        if not 'timestamp' in df:
            print(
                "[CO_model][disaggregate] Could not detect column \"timestamp\" in the given dataframe")
            return
    ...
    def save(self, filename):
        with open(filename+'.pkl', 'wb') as output:
            ...
            pickle.dump(self.state_combinations,
